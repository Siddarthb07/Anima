# Anima documentation index

Start here if you want depth beyond the root [`README.md`](../README.md).

Screenshots embedded in the root README live under [`images/`](images/) (`dashboard-readout-example.png`, `dashboard-websocket-troubleshooting.png`).

| Document | Audience |
|----------|----------|
| [GETTING_STARTED.md](GETTING_STARTED.md) | Anyone installing and running API + dashboard |
| [PROJECT_OVERVIEW.md](PROJECT_OVERVIEW.md) | Developers mapping modules, probes, TRIBE surrogate |
| [USAGE_AND_LIMITATIONS.md](USAGE_AND_LIMITATIONS.md) | Researchers, reviewers — allowed uses and hard boundaries |
| [RUN_AND_TEST_COMMANDS.txt](RUN_AND_TEST_COMMANDS.txt) | Commands for pytest, CLI, optional Playwright demo |
| [MODELS_AND_ZOO.md](MODELS_AND_ZOO.md) | Hugging Face models, Ollama clarification, probe files |
| [TRAINING.md](TRAINING.md) | Train text + brain probes |
| [TRAIN_ON_YOUR_MACHINE.md](TRAIN_ON_YOUR_MACHINE.md) | RAM, GPU, gated weights |
| [BENCHMARKS.md](BENCHMARKS.md) | Benchmark manifests |

Repository meta: [CONTRIBUTING.md](../CONTRIBUTING.md) · [CODE_OF_CONDUCT.md](../CODE_OF_CONDUCT.md) · [SECURITY.md](../SECURITY.md) · [LICENSE](../LICENSE)
