...
function QuadrantLabels() {
  return (
    <div className="pointer-events-none absolute inset-0 text-xs font-medium uppercase tracking-wide text-slate-500">
  ...
  if (!data.length) {
    return (
      <div className="relative flex h-80 items-center justify-center rounded-xl border border-white/10 bg-ink-900/40">
        <QuadrantLabels />
        <p className="text-base text-slate-500">Run a stream to plot valence × arousal.</p>
      </div>
    );
  }

  return (
    <div className="relative rounded-xl border border-white/10 bg-ink-900/40 p-4 shadow-inner">
      <QuadrantLabels />
      <h3 className="relative z-10 mb-2 text-base font-semibold text-slate-200">Circumplex (readout)</h3>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 12, right: 12, bottom: 20, left: 12 }}>
            ...
            <XAxis
              ...
              label={{ value: "Valence (− / +)", fill: "#64748b", fontSize: 13, position: "bottom" }}
            />
            <YAxis
              type="number"
              dataKey="arousal"
            ...
            <Tooltip
              ...
              content={({ active, payload }) => {
                ...
                return (
                  <div className="rounded-lg border border-white/10 bg-ink-950/95 px-3 py-2 text-sm shadow-xl">
                    ...
                    <div className="mt-2 grid grid-cols-3 gap-2 font-mono text-xs text-slate-400">
